...
def candidate(report):
 ...
 def disc(cx,cz,rx,rz,col,relief,steps=16):
  ...
  for i in range(steps):
   a=i*2*math.pi/steps;x=cx+rx*math.cos(a);z=cz+rz*math.sin(a);v.append((x,surface(x,z)+relief*.65,z))
  add(v,[(0,i+1,(i+1)%steps+1) for i in range(steps)],col)
 ...
 for s in (-1,1):
  ...
  strip([(cx-.033,1.677),(cx-.016,1.680),(cx+.018,1.680),(cx+.033,1.677)],.0065,7,.0035)
 ...
 for side,s in [('r',-1),('l',1)]:
  ...
  extra.vertex_groups.clear();group=extra.vertex_groups.new(name='hand_'+side);group.add(list(range(len(extra.data.vertices))),1,'REPLACE');extra.data.uv_layers.clear();uv=extra.data.uv_layers.new(name='UVMap')
