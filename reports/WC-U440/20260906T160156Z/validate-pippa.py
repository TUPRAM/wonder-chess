...
for name,spec in manifest['clips'].items():
    ...
    bpy.ops.import_scene.fbx(filepath=str(EXPORT/(spec['action']+'.fbx')),use_anim=True)
    ...
    path=REPORT/'pippa'/('continuous-'+name+'.mp4')
    ...
    clips.append(dict(clip=name,fbx_sha256=sha(EXPORT/(spec['action']+'.fbx')),imported_action=action.name,imported_frames=list(action.frame_range),bones=len(arms[0].data.bones),video_sha256=sha(path),video_frames=movie.frame_duration,video_fps=movie.fps,video_dimensions=list(movie.size)))
