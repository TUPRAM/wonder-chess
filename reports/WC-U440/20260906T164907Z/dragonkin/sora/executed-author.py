"""Sora pilot: original integrated draconic face, armor and seven contact poses.

Writes only the explicitly released Sora source/export paths and a fresh report.
...
from dragonkin_production_profile import RIG_REVISION, motion_pose
...
def head(g, profile, cls):
    ...
    for sign in (-1,1):
        ...
        g.tube([(sign*.065,-.015,.946),(sign*.080,-.050,.966),(sign*.085,-.096,.972)],
               [.020,.014,.0045],7,'head',10,depth=.75)
...
def geometry(unit,points):
    ...
    original_head, original_tube = prepared.head_geometry, Geometry.tube
    ...
    prepared.head_geometry = head; Geometry.tube = sleeve
    try: g,equipment_start = prepared.build_geometry(unit,points)
    finally: prepared.head_geometry = original_head; Geometry.tube = original_tube
    # Add chest articulation and a sewn blue tabard edge, all on named bones.
    for side,sign in (('l',1),('r',-1)):
        sh = Vector(points[side]['sh'])
        g.ellipsoid(tuple(sh),(.064,.067,.060),3,'clavicle_'+side,8,16)
...
def main():
    ...
    args = parser.parse_args(sys.argv[sys.argv.index('--')+1:]); report = args.report.resolve()
    ...
    for polygon,color in zip(mesh.data.polygons,geo.colors):
        if color in (1,3,6,7,14):polygon.use_smooth=True
    for obj in (arm,mesh):
    ...
    execution.UID=UID; execution.clip_spec=prepared.clip_contract; execution.motion_pose=motion_pose
    ...
    manifest=dict(status='AUTHORED_EXPORTED_BLENDER_EXECUTED_ENGINE_AND_VISUAL_ACCEPTANCE_OPEN',unit_id=UID,name=unit['name'],
        ...
        exported_coordinate_units='centimeters in independent temporary copies; source metres',engine_calibration=[],
        ...
        clips=clips,lods=lods,source_brief=unit['art'],equipment='Visual only; oval beacon shield and short ceremonial mace, fixed decorative gem',
