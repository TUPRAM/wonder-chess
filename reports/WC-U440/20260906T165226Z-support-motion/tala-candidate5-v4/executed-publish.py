...
def selected_pose(uid,arm,clip,frame,spec,h):
    ...
    if uid=='wc_u_elf_mage':
        ...
        errors=[hand('l',(.32,.14,.43),turn(7,0,-12))]
    else:
    ...
    return max(errors)
def candidate(uid,report,clips,skip_before=False):
    ...
    if not skip_before:clip_views(uid,report,clips,m,'before')
    for clip in clips:
...
def publish(uid,report):
    ...
    assert not json.loads((report/'candidate-structure.json').read_text())['errors'];stage=report/'normalized-clips';stage.mkdir(exist_ok=False);old=report/'retained-original-clips';old.mkdir(exist_ok=False)
    for clip in info['selected_clips']:
        spec=m['clips'][clip];name=spec['action']+'.fbx';shutil.copy2(folder/name,old/name);use_clip(arm,clip,1,unit_id=uid);bpy.context.scene.frame_start=spec['frames'][0];bpy.context.scene.frame_end=spec['frames'][1];export_normalized_copy(stage/name,[arm],True,export_fbx_raw)
        spec['maximum_support_reach_clamp_m']=info['maximum_hand_reach_clamp_m'][clip]
    use_clip(arm,'Idle',1,unit_id=uid);assert mesh_digest()==info['geometry_digest'];after={action.name:action_curve_digest(action) for action in bpy.data.actions};assert {name for name in after if after[name]!=info['before_action_hashes'][name]}==set(info['changed_actions']);bpy.ops.wm.save_as_mainfile(filepath=str(source),check_existing=False)
    for path in stage.glob('*.fbx'):shutil.copy2(path,folder/path.name)
    m.update({'source_revision':m['source_revision']+1,'animation_revision':m['animation_revision']+1,'source_sha256':sha(source),'motion_refinement_evidence':str(report),'motion_refinement_script_sha256':sha(report/'executed-candidate.py'),'motion_refinement_publish_sha256':sha(report/'executed-publish.py'),'motion_refinement':{'selected_clips':info['selected_clips'],'source_action_hashes':after,'geometry_digest_unchanged':info['geometry_digest'],'maximum_reach_clamp_m':info['maximum_hand_reach_clamp_m']},'files':{path.name:sha(path) for path in folder.iterdir() if path.is_file() and path.name!='export_manifest.json'}});(folder/'export_manifest.json').write_text(json.dumps(m,indent=2)+'\n');(report/'published-result.json').write_text(json.dumps({'status':'SOURCE_AND_SELECTED_CLIPS_EXPORTED_PENDING_READBACK','unit_id':uid,'source_revision':m['source_revision'],'animation_revision':m['animation_revision'],'source_sha256':sha(source),'manifest_sha256':sha(folder/'export_manifest.json'),'changed_clips':info['selected_clips']},indent=2)+'\n');print('WC_SELECTED_MOTION_PUBLISHED',uid)
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--unit',choices=['wc_u_elf_mage','wc_u_orc_guardian'],required=True);p.add_argument('--report',type=Path,required=True);p.add_argument('--clips',nargs='+');p.add_argument('--skip-before',action='store_true');p.add_argument('--publish',action='store_true');a=p.parse_args(sys.argv[sys.argv.index('--')+1:])
    if a.publish:publish(a.unit,a.report.resolve())
    else:assert a.clips;candidate(a.unit,a.report.resolve(),a.clips,a.skip_before)
