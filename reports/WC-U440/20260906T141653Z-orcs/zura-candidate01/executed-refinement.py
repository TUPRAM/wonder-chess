    ...
    if uid == 'wc_u_orc_warrior':
        ...
        for sign in (-1, 1):
            def position(u, v):
                ...
                z = .835 - .043 * u - .065 * u * u + .020 * math.sin(math.pi * u)
        ...
        for sign in (-1, 1):
            def position(u, v):
                x = sign * (.053 + .245 * u)
                y = (-.114 + .228 * v) * (.66 + .34 * u)
                z = .825 + .020 * math.sin(math.pi * v) - .110 * u ** 1.8 + .035 * math.sin(math.pi * u)
                return Vector((x, y, z)) * height
        ...
        def position(u, v):
            x = .069 + .222 * u
            y = -.17 + .34 * v
            z = 1.57 - .160 * u ** 1.7 + .030 * math.sin(math.pi * v)
            return (x, y, z)
        ...
        if groups == ['head'] and count == 112:
            ...
            for vertex in vertices:
                ...
                if uid.endswith('warrior') and groups == ['hand_l']:
                    vertex.co.z = center.z + (vertex.co.z - center.z) * 1.28 - .027
