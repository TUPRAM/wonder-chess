...
def contacts(u,arm,clip,frame,end,release):
    ...
    errors=[]
    if uid=='wc_u_human_warrior':
        rise=sample([0,.09,.07,.035,0]) if attack else sample([0,.145,.085,.035,0]) if active else .06*victory
        angle=sample([-142,-80,-135,-155,-142]) if attack else sample([-142,-42,-126,-150,-142]) if active else -142+52*victory
        point=(-.065,.22,.505+rise+.003*wave);rotation=turn(angle,0,-10)
        if defeat:point=(-.04,.22,.27);rotation=turn(-86,0,0)
        ...
        if defeat:
            errors=[hand('l',(.20,.20,.32),turn(-40)),hand('r',(-.20,.20,.28),turn(-35))]
    elif uid=='wc_u_dwarf_priest':
        ...
        if defeat:errors=[hand('r',(-.10,.26,.30),turn(-65)),hand('l',(.24,.12,.28),turn(-35))]
    else:
        ...
        if defeat:errors=[hand('l',(.24,.23,.31),turn(-77)),hand('r',(-.25,.17,.27),turn(-70))]
    return max(errors)
...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--unit',required=True,choices=sorted(ALLOWED));parser.add_argument('--report',type=Path,required=True);a=parser.parse_args(sys.argv[sys.argv.index('--')+1:]);uid=a.unit;report=a.report.resolve();report.mkdir(parents=True,exist_ok=False)
    ...
    assert not source.exists() and not out.exists(),'Existing progress requires incremental refinement, never regeneration'
