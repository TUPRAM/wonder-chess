...
p=argparse.ArgumentParser();p.add_argument('--unit',choices=('wc_u_dragonkin_guardian','wc_u_dragonkin_ranger','wc_u_dragonkin_rogue','wc_u_dragonkin_priest'),required=True)
...
if prior.exists() and ((uid=='wc_u_dragonkin_guardian' and manifest['source_revision']>=3) or (uid=='wc_u_dragonkin_ranger' and manifest['source_revision']>=2)):
    signatures=[]
    ...
    assert changed==(['Active'] if uid=='wc_u_dragonkin_guardian' and manifest['source_revision']==3 else []),('Unexpected changed actions',changed)
    preserved=dict(status='PASS_REST_AND_UNCHANGED_ACTIONS',before_source_sha256=sha(prior),
...
semantic=dict(continuous_gesture_quality_approved=False)
if uid=='wc_u_dragonkin_guardian':
    ...
    semantic.update(active_release_frame=manifest['clips']['Active']['release_frame'],
    ...
    strings=[part for part in components(mesh) if part['count']==18 and part['groups']==['weapon_l','weapon_r']]
    assert len(strings)==1,'Expected one continuous three-ring string with separately weighted nock'
    nock=[index for index in strings[0]['indices'] if mesh.vertex_groups[mesh.data.vertices[index].groups[0].group].name=='weapon_r']
    assert len(nock)==6
    max_gap=0.;draws={}
    for name,spec in manifest['clips'].items():
        sampled=[]
        for frame in range(1,spec['frames'][1]+1):
            use_clip(arm,spec,frame);obj=mesh.evaluated_get(bpy.context.evaluated_depsgraph_get());evaluated=obj.to_mesh()
            center=sum((obj.matrix_world@evaluated.vertices[index].co for index in nock),Vector())/6
            max_gap=max(max_gap,(center-arm.pose.bones['hand_r'].tail).length)
            sampled.append(arm.pose.bones['hand_l'].tail.y-center.y);obj.to_mesh_clear()
        if name in ('Attack','Active'):draws[name]=max(sampled)-min(sampled)
    assert max_gap<.0001,('Nock detached from drawing hand',max_gap)
    assert all(value>.04*unit['height_m'] for value in draws.values()),('No actual draw/release motion',draws)
    semantic.update(nock_maximum_hand_gap_m=max_gap,draw_range_m=draws,source_frames_checked=sum(s['frames'][1] for s in manifest['clips'].values()))
elif uid=='wc_u_dragonkin_rogue':
    windows=manifest['clips']['Attack'].get('presentation_windows',[])
    assert [w['name'] for w in windows]==['R','L']
    assert windows[0]['end_frame']==windows[1]['start_frame'] and windows[0]['start_frame']==1
    assert windows[1]['end_frame']==manifest['clips']['Attack']['frames'][1]
    movements=[]
    for window in windows:
        assert window['release_frame']-window['start_frame']==round(unit['stats']['attack_windup_ms']*.06)
        use_clip(arm,manifest['clips']['Attack'],window['start_frame'])
        start={side:arm.pose.bones['hand_'+side].tail.copy() for side in ('r','l')};distance={'r':0.,'l':0.}
        for frame in range(window['start_frame'],window['end_frame']+1):
            use_clip(arm,manifest['clips']['Attack'],frame)
            for side in distance:distance[side]=max(distance[side],(arm.pose.bones['hand_'+side].tail-start[side]).length)
        side=window['name'].lower();other='l' if side=='r' else 'r'
        assert distance[side]>.05 and distance[other]<distance[side]*.25,(window,distance)
        movements.append(dict(window=window,hand_movement_m=distance))
    semantic.update(two_actual_alternating_attack_windows=movements,attack_asset_count=1)
else:
    use_clip(arm,manifest['clips']['Idle']);base=arm.pose.bones['hand_l'].tail.z
    use_clip(arm,manifest['clips']['Active'],manifest['clips']['Active']['release_frame'])
    rise=arm.pose.bones['hand_l'].tail.z-base
    assert rise>.1*unit['height_m'],('Sky Ward free palm fails to rise',rise)
    semantic.update(free_palm_rise_m=rise,active_release_frame=manifest['clips']['Active']['release_frame'])
