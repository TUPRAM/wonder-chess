...
assert manifest['source_revision']==3,'Expected the engine-calibrated Sora3 source'
...
bpy.ops.wm.open_mainfile(filepath=str(source_file));scene=bpy.context.scene
...
updated=dict(manifest);updated.update(source_revision=4,geometry_revision=4,geometry_source_revision=4,
...
write(report/'refinement.json',dict(status='COSTUME_ONLY_REFINEMENT_EXECUTED',source_before_sha256=manifest['source_sha256'],
    source_after_sha256=updated['source_sha256'],preserved_invariants=before,verified_candidate_invariants=invariants(arm),
...
write(report/'completed.json',dict(status='SOURCE_EXPORT_FROZEN_PENDING_UNREAL_REIMPORT',unit_id=UID,
    source_revision=4,source_sha256=updated['source_sha256'],export_manifest_sha256=sha(out/'export_manifest.json')))
