...
def root_feet(g, count=2, radius=.28, y=.02):
    ...
    for i in range(count):
        ...
        if count==4:
            g.tube([(x*.40,yy*.40,.29),(x*.75,yy*.78,.32),(x,yy,.36)],
                   [.10,.095,.095],5,bone,10)
...
def stoneback():
    ...
    for band in range(3):
        ...
        for i in range(sectors):
            ...
            g.add(verts,[(0,1,2,3),(7,6,5,4),(0,4,5,1),(1,5,6,2),(2,6,7,3),(3,7,4,0)],
                  2 if (i+band)%3 else 3,'shell_l' if math.cos((a0+a1)/2)<0 else 'shell_r')
