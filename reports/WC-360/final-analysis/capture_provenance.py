...
def main():
    ...
    if stage["alpha_unit_ids"] != alpha_ids or len(alpha_ids) != 12 or len(set(alpha_ids)) != 12:
        raise ValueError("Staged alpha roster does not match the twelve canonical IDs")
    comparisons = []
    ...
    for unit_id in stage["alpha_unit_ids"]:
        ...
        if capture(ROOT / "data/units.json", "canonical_data")["sha256"] != manifest["units_source_sha256"]:
            raise ValueError("Hero manifest points to different canonical units: " + unit_id)
        files = []
    ...
    result = {"schema": "wonder_chess_candidate_provenance_v2", "candidate": args.candidate,
              ...
              "art_boundary": "Current authored source/export hashes checked; no implication of visual approval from hashing.",
              ...
              "hero_export_comparisons": art_comparisons, "groups": groups,
