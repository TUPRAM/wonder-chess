"""Audit actual startup-fixed Shipping regression against preserved engine and native rows."""
...
def main():
    report_path = ROOT / "reports/WC-360/shipping-startup-fixed-regression/regression.json"
    provenance_path = ROOT / "reports/WC-360/shipping-startup-fixed-provenance.json"
    ...
    lines += ["", "Runtime-loaded profile and all five canonical data hashes agree with shipping-startup-fixed-provenance.json. The actual Shipping inner executable SHA256 matches that immutable manifest.", "",
              result["exit_boundary"], "",
              "This audit binds the actual startup-fixed Shipping execution to its immutable source/data/content/package manifest. The earlier Shipping run and its failed network startup trial remain preserved separately.", ""]
