...
$script:LaunchCalls = [Collections.Generic.List[object]]::new()
...
function Start-Process {
    ...
    $script:LaunchCalls.Add([ordered]@{file=$FilePath;arguments=$ArgumentList})
...
function Invoke-RefusalCase {
    ...
    $CallsBefore=$script:LaunchCalls.Count
    $ErrorText=''
    try { & $Wrapper @Parameters | Out-Null } catch { $ErrorText=$_.Exception.Message }
    $Calls=$script:LaunchCalls.Count-$CallsBefore
    ...
    $Rows.Add([ordered]@{name=$Name;pass=$Pass;actual_error=$ErrorText;expected_error_contains=$Expected;
      ...
      environment_preserved=$EnvironmentPreserved;existing_marker_hash_preserved=$HashPreserved;marker_sha256=$Before})
}
...
Invoke-RefusalCase wrong-case-task @{Task='IMPORT_ALPHA_ASSETS'} 'ValidateSet'
...
$Result=[ordered]@{status=$(if($Failures){'FAIL'}else{'PASS'});executed_utc=[DateTime]::UtcNow.ToString('o');
    ...
    tests=$Rows.Count;passed=$Rows.Count-$Failures;failed=$Failures;editor_processes_launched=0;intercepted_launch_calls=$script:LaunchCalls;
    checks=$Rows}
