...
def render(u):
 ...
 magnitude=[(f'{v/100:g}%' if a['magnitude_unit']=='basis_points' else points(v) if a['magnitude_unit']=='centipoints' else 'Not applicable') for v in a['magnitude_by_star']]
 lines=[f'# {u["name"]} — {u["title"]}',f'**{u["race"].title()} · {u["unit_class"].title()} · {u["role"]} · {u["production_phase"].upper()}**',
 f'Stable identity: `{u["id"]}`. Cost: **{u["cost"]} gold**. Rarity label: **{u["rarity"]}**, separate from star level. Status: designed, not modeled or implemented by this kit. All combat values are provisional prototype inputs.',
 ...
 '| Contract field | Value |','|---|---|',f'| Stable ability ID | `{a["id"]}` |',f'| Effect / target selector | `{a["effect"]}` / `{a["target_rule"]}` |',f'| One / two / three star magnitude | {" / ".join(magnitude)} |',f'| Damage type | {a["damage_type"] or "Not damaging"} |',f'| First-cast delay / cooldown | {a["first_cast_ms"]/1000:g} s / {a["cooldown_ms"]/1000:g} s |',f'| Windup / recovery | {a["cast_ms"]/1000:g} s / {a["recovery_ms"]/1000:g} s |',f'| Effect duration | {a["duration_ms"]/1000:g} s |',f'| Skill reach / area radius | {a["range_tiles"]} / {a["radius_tiles"]} tiles |',f'| Can include self | {a["allow_self"]} |',f'| Max dash distance | {a["max_dash_tiles"]} tiles |',f'| Projectile travel | {a["travel_ms"]} ms |',f'| Affected stat | {a["stat"] or "Not applicable"} |',
...
def main():
 ...
 index=['# Wonder Chess — Complete Character Bible v3','All 24 heroes are fully described. The first twelve are the playable-alpha design set; the remaining twelve are expansion designs, not initial asset obligations. Numbers are prototype values, not validated balance.','## Master roster','| Hero | Race | Class | Cost | Phase | Active skill |','|---|---|---|---:|---|---|']
