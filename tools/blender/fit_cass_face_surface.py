...
def candidate(report):
 ...
 for s in (-1,1):
  cx=s*.0663;cz=1.6889;disc(cx,cz,.033,.0115,8,.0022);disc(cx,cz,.010,.0103,9,.0037);disc(cx,cz,.0052,.0054,15,.0046);disc(cx-.0027,cz+.0035,.0014,.0015,8,.0052);lid(cx,cz,True);lid(cx,cz,False)
  brow=[]
  for i in range(17):
   dx=-.042+i*.084/16;x=s*.0678+dx;z=1.737-.006*abs(dx/.042)
   for dz in (-.0065,.0065):brow.append((x,surface(x,z+dz)+.0035,z+dz))
  add(brow,[(j,j+1,j+3,j+2) for j in range(0,len(brow)-2,2)],7)
 # Keep the authored pale streak over Cass's left brow, fitted just above its surface.
 patch=[]
 for j in range(5):
  z=1.727+j*.017/4
  for i in range(5):
   x=.0769+i*.016/4;patch.append((x,surface(x,z)+.0049,z))
 add(patch,[(j*5+i,j*5+i+1,(j+1)*5+i+1,(j+1)*5+i) for j in range(4) for i in range(4)],1)
 changed=[]
 for ob in (mesh,*list(bpy.data.collections['BODY'].objects)):
  ...
  for c in components(ob):
   ...
   if c['count'] in (18,12) and palette in (7,1) and c['center_m'][2]>1.72:
    remove.update(c['indices']);changed.append({'object':ob.name,'brow_or_streak_vertices_rebuilt_on_surface':c['count']})
  if remove:
 ...
 assert sha(SOURCE)==m['source_sha256'];(report/'candidate-result.json').write_text(json.dumps({'status':'ACTUAL_CASS_CANDIDATE_RENDERED_NOT_PUBLISHED','source_before_sha256':m['source_sha256'],'candidate_sha256':sha(report/'cass-source5-candidate.blend'),'preserved_invariants':saved,'changes':changed,'preserved':['Head shell, nose, mouth, ears and hair geometry','Brow expression and pale streak identity retained through fitted surface geometry','All costume and equipment meshes','All seven action curves and rest skeleton']},indent=2)+'\n');print('WC_CASS_FACE_CANDIDATE_RENDERED',flush=True)
def publish(report):
