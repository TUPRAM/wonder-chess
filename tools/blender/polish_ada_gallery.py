...
def candidate(report):
 ...
 for side,s in [('r',-1),('l',1)]:
  ...
  if side=='r':
   # A broad palm pad joins the knuckle roots; it is fused by remesh, not left as another intersecting object.
   hand.loft([((s*.5824/h,.080/h,z/h),w/h,d/h) for z,w,d in ((.697,.030,.022),(.711,.053,.035),(.735,.057,.040),(.779,.057,.040),(.812,.045,.034),(.829,.024,.019))],6,'hand_r',14)
   for j,z in enumerate((.714,.738,.762,.786)):
    pts=[(-.5824+.047*math.cos(a),.1001+.044*math.sin(a),z) for a in [math.radians(230-i*280/19) for i in range(20)]];hand.tube([tuple(q/h for q in p) for p in pts],[r/h for r in ([.0105]+[.0125]*18+[.0105])],6,'hand_r',10)
  ...
  smooth=extra.modifiers.new('Sculpt_surface_relax','SMOOTH');smooth.factor=.40;smooth.iterations=3;bpy.ops.object.modifier_apply(modifier=smooth.name);extra.data.calc_loop_triangles();dec=extra.modifiers.new('Sculpt_faceted_finish','DECIMATE');dec.ratio=min(1,2000/len(extra.data.loop_triangles));bpy.ops.object.modifier_apply(modifier=dec.name)
  ...
  copy=extra.copy();copy.data=extra.data.copy();bpy.data.collections['BODY'].objects.link(copy);body=join(body,copy);body.hide_render=True;body.hide_set(True);mesh=join(mesh,extra);sculpted.append({'side':side,'voxel_size_m':.0035,'surface_relax_iterations':3,'deform_bone':'hand_'+side})
