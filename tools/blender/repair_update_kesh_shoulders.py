"""Connect Kesh existing sleeve roots while retaining every animation and the rest rig."""
...
def candidate(uid,report):
    ...
    for ob in [bpy.data.objects[name] for name in ['SK_'+uid,'BODY_SK_'+uid,'COSTUME_SK_'+uid]]:
    ...
    (report/'sleeve-change-selection.json').write_text(json.dumps(changes,indent=2)+'\n');assert len(changes)==4
    after=invariants(arm);check_invariants(before,after,uid);after_hashes=action_hashes();changed=[name for name in hashes if hashes[name]!=after_hashes[name]];assert changed==[];use_clip(arm,'Idle',unit_id=uid);bpy.ops.wm.save_as_mainfile(filepath=str(report/'candidate.blend'),check_existing=False);render_views(uid,report,h)
    assert sha(source)==m['source_sha256'];(report/'candidate-result.json').write_text(json.dumps({'status':'ACTUAL_SLEEVE_REPAIR_CANDIDATE_RENDERED_NOT_PUBLISHED','unit_id':uid,'source_before_sha256':m['source_sha256'],'candidate_sha256':sha(report/'candidate.blend'),'before_invariants':before,'after_invariants':after,'before_action_hashes':hashes,'after_action_hashes':after_hashes,'changed_actions':changed,'changes':changes,'preserved':['Mantle/hair/palette/gear and every sleeve vertex beyond the two root rings','Rest skeleton and canonical clip timing','All seven Kesh actions and existing knife/cloth geometry'],'motion_boundary':'All7 actions are byte-for-byte equivalent by curve digest; only existing sleeve root coordinates and weights change.'},indent=2)+'\n');print('WC_KESH_SLEEVE_CANDIDATE',uid)
def publish(uid,report):
    ...
    shutil.copy2(report/'three-quarter.png',out/'portrait.png');mesh.data.calc_loop_triangles();m.update({'source_revision':8,'geometry_source_revision':8,'geometry_revision':8,'animation_revision':m['animation_revision'],'source_sha256':sha(source),'triangles':len(mesh.data.loop_triangles),'lods':lods,'update_refinement_script_sha256':sha(report/'executed-repair.py'),'update_export_script_sha256':sha(report/'executed-publish.py'),'update_refinement_evidence':str(report),'shoulder_repair':{'changed_actions':info['changed_actions'],'source_action_hashes':info['after_action_hashes'],'boundary':info['motion_boundary']},'files':{p.name:sha(p) for p in out.iterdir() if p.is_file() and p.name!='export_manifest.json'}});(out/'export_manifest.json').write_text(json.dumps(m,indent=2)+'\n');(report/'published-result.json').write_text(json.dumps({'status':'KESH_SOURCE8_EXPORTED_PENDING_FRESH_VERIFICATION','unit_id':uid,'source_sha256':sha(source),'manifest_sha256':sha(out/'export_manifest.json'),'triangles':m['triangles'],'lods':lods,'changed_actions':info['changed_actions']},indent=2)+'\n');print('WC_KESH_SLEEVE_PUBLISHED',uid)
p=argparse.ArgumentParser();p.add_argument('--unit',choices=['wc_u_orc_rogue'],required=True);p.add_argument('--report',type=Path,required=True);p.add_argument('--publish',action='store_true');a=p.parse_args(sys.argv[sys.argv.index('--')+1:]);publish(a.unit,a.report.resolve()) if a.publish else candidate(a.unit,a.report.resolve())
