...
def face_hair(g,u):
    ...
    for s in (-1,1):
        x=s*width*.44;y=.084
        def eye_point(dx,z,relief):return (x+dx,.006+.079*math.sqrt(max(.01,1-((x+dx)/width)**2))+relief,z)
        g.add([eye_point(-.017,.907,.0015),eye_point(-.008,.913,.0015),eye_point(.011,.913,.0015),eye_point(.017,.907,.0015),eye_point(.006,.902,.0015),eye_point(-.011,.903,.0015)],[(0,1,2,3,4,5)],8,'head')
        g.add([eye_point(-.005,.912,.0025),eye_point(.006,.912,.0025),eye_point(.006,.903,.0025),eye_point(-.005,.903,.0025)],[(0,1,2,3)],9,'head')
        g.add([eye_point(-.0015,.910,.0035),eye_point(.0025,.910,.0035),eye_point(.0025,.904,.0035),eye_point(-.0015,.904,.0035)],[(0,1,2,3)],15,'head')
        g.tube([eye_point(-.019,.908,.002),eye_point(-.007,.916,.002),eye_point(.012,.915,.002),eye_point(.020,.908,.002)],[.003]*4,14,'head',7)
...
def body(g,u,p):
    ...
    for side in ('l','r'):
        ...
        g.tube([(s*sh*.64,0,q['sh'][2]-.033),q['sh'],tuple(a.lerp(b,.22))],[radius*.90,radius*.93,radius*.96],0,'upperarm_'+side,16,weights=[{'spine_03':1},{'spine_03':.28,'upperarm_'+side:.72},{'upperarm_'+side:1}])
...
def contacts(u,arm,clip,frame,end,release):
    ...
    if uid=='wc_u_human_warrior':
        ...
        if defeat:point=tuple(Vector((-.025,.115,.665)).lerp(Vector((.015,.10,.35)),kneel**1.5));rotation=turn(-142+56*kneel,0,-10+10*kneel)
        ...
        errors.append(hand('r',(-.22+.13*a,.10+.20*a+.03*f,.48+.26*a+.06*f+.12*victory),turn(-12-45*a-18*f,0,12*a)))
        if defeat:
...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--unit',required=True,choices=sorted(ALLOWED));parser.add_argument('--report',type=Path,required=True);parser.add_argument('--resume-partial',action='store_true');parser.add_argument('--resume-source',type=Path);a=parser.parse_args(sys.argv[sys.argv.index('--')+1:]);uid=a.unit;report=a.report.resolve();report.mkdir(parents=True,exist_ok=False)
    ...
    lods=[]
    if a.resume_source:
        assert a.resume_partial and a.resume_source.is_file()
        (report/'resumed-source.json').write_text(json.dumps({'path':str(a.resume_source.resolve()),'sha256':sha(a.resume_source)},indent=2))
        bpy.ops.wm.open_mainfile(filepath=str(a.resume_source.resolve()));scene=bpy.context.scene;arm=bpy.data.objects['Armature'];mesh=bpy.data.objects['SK_'+uid];assert abs(scene.unit_settings.scale_length-1)<1e-8
        mesh.data.calc_loop_triangles()
        for index,ratio in ((1,.5),(2,.25)):
            lod=bpy.data.objects[f'SK_{uid}_LOD{index}'];lod.data.calc_loop_triangles();lods.append({'lod':index,'triangles':len(lod.data.loop_triangles),'ratio_target':ratio,'visual_acceptance':'pending'})
    else:
        ...
        lod_collection=bpy.data.collections.new('LOD_SOURCE');scene.collection.children.link(lod_collection)
        for index,ratio in ((1,.5),(2,.25)):
