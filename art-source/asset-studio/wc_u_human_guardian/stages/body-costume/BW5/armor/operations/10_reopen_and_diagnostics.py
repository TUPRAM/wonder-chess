...
R=Path(__file__).resolve().parents[1];frozen=R/'ada_bw5_armor_checkpoint_ART_REVISE.blend';work=R/'ada_bw5_armor_work.blend'
def signature(o):
...
assert all(fs[n]==v for n,v in c2.items())
...
(R/'records/critical_subframes.json').write_text(json.dumps({'source':str(frozen),'sha256':hashlib.sha256(frozen.read_bytes()).hexdigest(),'frames':extra,'scope':'Eight additional quarter-frame transverse crossing checks, not continuous collision certification'},indent=2))
report={'source_work':str(work),'work_sha256':hashlib.sha256(work.read_bytes()).hexdigest(),'frozen':str(frozen),'frozen_sha256':hashlib.sha256(frozen.read_bytes()).hexdigest(),'work_frozen_mesh_signatures_match':True,'c2_geometry_signatures_match_retained_frozen':True,'all97_c2_audit_applies_to_unchanged_geometry_and_same_action':True,'unchanged_original_and_context_mesh_key_records':len(preserve),'historical_cameras_exactly_unchanged':True,'five_sampled_authoring_pose_matrix_max_difference':poseerr,'solidify_preserved':sol,'parts':struct,'reference_scene_has_independent_rig_data':bpy.data.objects['BW5_ReferencePose_Rig'].data!=bpy.data.objects['BW4_Armor_Independent_Rig'].data,'status':'ART_REVISE','not_run':['Canonical seven game clips','Unreal import/reimport','Packaged game','Human approval','Recipe promotion']}
...
for label,axis,deg in [('forward_bend','X',-10),('torso_twist','Z',15)]:
    ...
    for view in ['front','right','back']:
        rs.camera=bpy.data.objects['BW5_Reference_'+view];rs.render.filepath=str(R/'captures'/f'diagnostic_{label}_{view}.png');bpy.ops.render.render(write_still=True)
pb.matrix_basis=basis
(R/'records/supplementary_bending.json').write_text(json.dumps({'source':str(frozen),'scope':'Two static deliberate world-axis pose tests on independent reference rig. Not original authoring action, not actual game clips or validated anatomical range. No source save.','tests':tests},indent=2))
