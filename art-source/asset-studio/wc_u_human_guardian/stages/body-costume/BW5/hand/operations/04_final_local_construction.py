...
layout={2:(.026,.0095,.090,.0065,.0095),3:(.003,.0105,.092,.006,.010),4:(-.019,.0096,.090,.006,.0095),5:(-.0375,.008,.086,.006,.0085)}
...
for n,(xc,wx,yc,zc,wz) in layout.items():
 ...
 # Independent anatomical decisions: longest middle, descending ring/little,
 # and a broader proximal middle knuckle. No uniform strand replication.
 front,top,tipy,tipz={2:(.112,.058,.067,.044),3:(.114,.060,.064,.043),4:(.112,.0585,.066,.044),5:(.110,.0555,.074,.049)}[n]
 sections=[(yc,zc,0,1),(.103,.010,math.radians(30),1.00),(front,.029,math.radians(77),1.00),(front-.005,.049,math.radians(130),.98),(.088,top,math.radians(182),.90),(tipy+.006,tipz+.008,math.radians(226),.83),(tipy,tipz+.0015,math.radians(250),.64),(tipy-.002,tipz,math.radians(250),.30)]
...
for row,t in enumerate((0,.2,.45,.70,.86,1)):
 ...
 for j,(ti,bi) in enumerate(zip(topfront,bottomfront)):
  ...
  lateral=abs(xf/.049)
  ztop=.018*(1-.28*lateral**2)-.006*t+.004*math.sin(t*math.pi)
  # Named thenar control rows, confined behind the grip; distinct palm volume.
  if row in (1,2,3):ztop+=.007*max(0,1-abs(x-.026)/.025)*max(0,1-abs(y-.052)/.030)
  zbot=-.018*(1-.18*lateral**2)+.011*t
  if row==4:
   ztop=.85*ztop+.15*V[ti][2];zbot=.85*zbot+.15*V[bi][2]
...
remove=[]
for k in (1,2):
 remove.append(set([toprows[k][right],toprows[k+1][right],bottomrows[k+1][right],bottomrows[k][right]]))
 for rows in (toprows,bottomrows):
  for j in range(inner,right):remove.append(set([rows[k][j],rows[k][j+1],rows[k+1][j+1],rows[k+1][j]]))
removed=[f for f in F if set(f) in remove]
F[:]=[f for f in F if set(f) not in remove]
from collections import Counter
bc=Counter(tuple(sorted((a,b))) for f in removed for a,b in zip(f,f[1:]+f[:1]))
# Boundary of removed faces, excluding edges without any retained neighbor.
keepedges={tuple(sorted((a,b))) for f in F for a,b in zip(f,f[1:]+f[:1])}
adj=defaultdict(list)
for (a,b),count in bc.items():
 if count==1 and (a,b) in keepedges:adj[a].append(b);adj[b].append(a)
holecorners=[];a=next(iter(adj));start=a;previous=None
for _ in range(100):
 holecorners.append(a);nxt=next(i for i in adj[a] if i!=previous);previous,a=a,nxt
 if a==start:break
assert len(holecorners)==len(adj)
...
prev=holecorners
thumb_sections=[((.044,.048,.006),(.016,.013)),((.044,.053,.018),(.012,.011)),((.037,.059,.029),(.009,.009)),((.026,.0622,.033),(.008,.008)),((.017,.0632,.032),(.006,.006)),((.012,.0632,.030),(.003,.003))]
...
o=bpy.data.objects.new('BW5_ClosedGlove_Correction2',me);s.collection.objects.link(o);o.color=(.55,.53,.50,1)
for p in me.polygons:p.use_smooth=True
sub=o.modifiers.new('Retained review subdivision 1','SUBSURF');sub.levels=1;sub.render_levels=1
declared={}
for n in (2,3,4,5):
 ids=regions[f'{n}_new_digit']
 chosen=[ids[station*16+k] for station in (5,6) for k in (2,3,4,5,6)]
 declared[str(n)]={'anatomy':{2:'index',3:'middle',4:'ring',5:'little'}[n],'construction_region':'Distal palmar sector, stations 5 and 6, profile vertices 2 through 6; declared before candidate scoring','vertices':[remap[i] for i in chosen]}
 vg=o.vertex_groups.new(name='BW5_CONTACT_'+declared[str(n)]['anatomy']);vg.add(declared[str(n)]['vertices'],1,'REPLACE')
thumbchosen=[thumbids[station*16+k] for station in (3,4) for k in (14,15,0,1,2)]
declared['1']={'anatomy':'thumb','construction_region':'Opposing distal pad, thumb stations 3 and 4, profile sector 14 15 0 1 2; declared before scoring','vertices':[remap[i] for i in thumbchosen]}
vg=o.vertex_groups.new(name='BW5_CONTACT_thumb');vg.add(declared['1']['vertices'],1,'REPLACE')
(OUT/'records/contact_regions_declared_before_final_audit.json').write_text(json.dumps(declared,indent=2))
...
s.name='BW5_HAND_OUTSIDE_CHANNEL_CORRECTION2';s['BW5_scope']='Fixed glove local construction only. No human approval or runtime acceptance.'
for ob in s.objects:
 if ob.name.startswith('BW4_SelectedSword_'):ob.hide_render=False
s.camera=bpy.data.objects['BW4_oblique']
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'ada_bw5_hand_correction2.blend'))
for name in ('palm','dorsal','side','underside','oblique','axial'):
 ...
 if name in ('palm','oblique','side','axial'):
  ...
  s.render.filepath=str(OUT/'captures'/('correction2_hilt_only_'+name+'.png'));bpy.ops.render.render(write_still=True)
  for ob in s.objects:
   if ob.name.startswith('BW4_SelectedSword_'):ob.hide_render=False
record={'source':str(BW4/'ada_fixed_hand_checkpoint_ART_REVISE.blend'),'source_sha256':hashlib.sha256((BW4/'ada_fixed_hand_checkpoint_ART_REVISE.blend').read_bytes()).hexdigest(),'attempt':'correction2','new_vertices':len(me.vertices),'new_faces':len(me.polygons),'old_faces_retained':0,'retained_distal':{},'anatomical_digit_construction':regions,'proximal_sections':route_record,'old_to_new_evaluated_ids_reused':False,'handle_registration':'Unchanged BW4 selected hilt and rigid placement; 30x26 mm, 110 mm exposed, original guard/blade','representation':'Visible CC0 glove derivative, no underlying hand deletion or bare-hand repair claim','runtime':'NOT_RUN_LOCAL_GATE_REQUIRED'}
(OUT/'records/correction2_construction.json').write_text(json.dumps(record,indent=2))
