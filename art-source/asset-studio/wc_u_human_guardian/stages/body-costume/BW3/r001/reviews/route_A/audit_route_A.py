"""Read-only routeA geometry queries. No fitting, pose overrides, or source saves."""
import bpy
import ast
import hashlib
import json
...
EXPECTED = 'f7d42c9f6c04948d54745439de5a288979c2cbb922e11d0b64386d80c0aa1a3a'
...
def measure(obj, frame):
    ...
    result = {'frame': frame, 'object': obj.name, 'evaluated_vertices': len(q),
    ...
    print('SAMPLE', obj.name, frame, 'crossings', result['crossing_pair_count'], result['categories'],
...
report = {'status': 'RUNNING', 'source': str(SOURCE), 'source_sha256': EXPECTED,
          ...
          'method': 'Reuse checked BW2 triangle-slab and nonadjacent transverse crossing queries; actual derived evaluated body/glove. Thumb authored route only, digits2-5 asserted open. No pose overrides or optimization.',
...
for obj in (glove, body):
    variant = {'object': obj.name, 'integer_frames': [], 'half_frames': [], 'endpoint145': None, 'first_onsets': {}}
    report['variants'].append(variant)
    for frame in range(1, 26):
        variant['integer_frames'].append(measure(obj, frame))
        (OUT / 'route_A_results.json').write_text(json.dumps(report, indent=2) + '\n')
    halves = set()
    ...
    for frame in sorted(halves):
        variant['half_frames'].append(measure(obj, frame))
    variant['endpoint145'] = measure(obj, 145)
