...
tile_w, tile_h, label_h, header_h = 230, 230, 28, 55
inventory = []
for start in range(1, 146, 30):
    ...
    draw.text((12, 11), f'BW2 fixed handle | frames {start:03}-{end:03} | 24fps | ART_REVISE', font=headerfont, fill=(238, 238, 238))
    for offset, frame in enumerate(range(start, end + 1)):
        ...
        phase = 'open/close' if frame < 25 else 'held'
...
manifest = {
    ...
    'source_blend_sha256': 'd64d5900317541710d5ef29939d12559e5ecef31777a7e9261158cbccb7a41cf',
...
with (out / 'contact_sheet_manifest.json').open('x', encoding='utf-8') as stream:
