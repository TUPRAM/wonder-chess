...
out=Path(__file__).parent;record=json.loads((out/'bounded_fit.json').read_text());source=Path(record['source']);assert hashlib.sha256(source.read_bytes()).hexdigest()==record['source_sha256']
...
results=[]
for label,angles,z in [('initial',[15.2,59.4,56.8],0),('best',record['best_feasible']['angles_X_deg'],record['best_feasible']['root_Z_abduction_deg'])]:
    for i,a in enumerate(angles):p=r.pose.bones[f'finger3-{i+1}.R'];p.rotation_mode='XYZ';p.rotation_euler=(math.radians(a),0,math.radians(z) if i==0 else 0)
