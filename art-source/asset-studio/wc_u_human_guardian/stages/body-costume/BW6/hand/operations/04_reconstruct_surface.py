...
source=OUT/'ada_bw6_hand_target_correction2.blend';bpy.ops.wm.open_mainfile(filepath=str(source),load_ui=False,use_scripts=False)
...
bpy.ops.object.quadriflow_remesh(target_faces=1600,use_mesh_symmetry=False,use_preserve_sharp=False,use_preserve_boundary=False)
...
s.camera=bpy.data.objects['BW4_oblique'];bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'ada_bw6_hand_reconstructed_initial.blend'))
for name in ('palm','dorsal','side','oblique','axial','underside'):
 s.camera=bpy.data.objects['BW4_'+name];s.render.filepath=str(OUT/'captures'/('reconstructed_initial_'+name+'.png'));bpy.ops.render.render(write_still=True)
 if name in ('palm','oblique','side'):
  for ob in s.objects:
   if ob.name.startswith('BW4_SelectedSword_'):ob.hide_render=True
  s.render.filepath=str(OUT/'captures'/('reconstructed_initial_'+name+'_isolated.png'));bpy.ops.render.render(write_still=True)
  for ob in s.objects:
   if ob.name.startswith('BW4_SelectedSword_'):ob.hide_render=not ob.name.endswith('handle')
(OUT/'records/reconstructed_initial.json').write_text(json.dumps({'target_source':str(source),'target_sha256':hashlib.sha256(source.read_bytes()).hexdigest(),'new_cage_vertices':len(o.data.vertices),'new_cage_faces':len(o.data.polygons),'target_faces_requested':1600,'source_wrist_vertices':20,'boundary_reconstruction':'One cut loop at Y25 mm, angular correspondence bridge to exact source wrist','topology_correspondence_to_old_BW5':False,'human_approval':False,'rig_runtime':'NOT_RUN'},indent=2))
