"""Read-only BW6 surface query; exact existing BW2/BW4 query functions."""
...
o=next(o for o in bpy.context.scene.objects if o.name.startswith('BW6_ClosedGlove_'))
...
report={'source':str(source),'source_sha256':before,'object':o.name,'query_basis':'Existing BW2 confirmed transverse triangle query and BW4 convex-hilt signed samples; no ideal cylinder','modes':{},'contact':'SCREENED_PROVISIONAL_ANATOMICAL_REGIONS; no fit or approval based on these regions' if any(g.name.startswith('BW6_CONTACT_') for g in o.vertex_groups) else 'NOT_SCORED_NO_NEW_ANATOMICAL_MAPPING','limits':['Shared-vertex triangle pairs excluded','Coplanar and tangent cases not classified','Vertex SDF is a sample, not maximum full triangle depth','Intended wrist opening means glove is not a closed solid','New contact patches are provisional semantic selections, not approved pad correspondence']}
...
for raw in (True,False):
 ...
 for name in ('index','middle','ring','little','thumb'):
  ids=[i for i,weight in enumerate(w) if weight.get('BW6_CONTACT_'+name,0)>.999]
  distances=ds[ids]
  pads[name]={'selection':'Predeclared anatomical distal target region, threshold > .999 after subdivision; no closest-vertex reselection','count':len(ids),'indices':ids,'points_m':q[ids].tolist(),'distance_mm':distances.tolist(),'summary_mm':{'min':float(distances.min()),'median':float(np.median(distances)),'max':float(distances.max())} if len(ids) else None,'count_in_minus0_5_plus1mm_band':int(((distances>=-.5)&(distances<=1)).sum())}
 entry['contact_regions']=pads
 print('BW6_CHECK',mode,'self',own['confirmed_nonadjacent_transverse_pairs'],'equipment',{p:r['confirmed_transverse_pairs'] for p,r in checks.items()},'min',float(ds.min()),flush=True)
