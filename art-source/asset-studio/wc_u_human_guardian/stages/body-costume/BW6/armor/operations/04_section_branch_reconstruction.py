...
for v in coat.data.vertices:
 p=v.co.copy();z=p.z
 if 1.055<z<1.50 and abs(p.x)<.225:
  weight=smooth((z-1.055)/.055)*smooth((1.50-z)/.055)*smooth((.225-abs(p.x))/.05)
  center=Vector((0,-.025,z));direction=Vector((p.x,p.y+.025,0))
  if direction.length<1e-6:continue
  direction.normalize();hit=bodybv.ray_cast(center,direction,.31)
  if hit[0] is not None:
   q=hit[0]+direction*.020
   v.co=p.lerp(q,weight)
   changes.append({'vertex':v.index,'before':list(p),'after':list(v.co),'purpose':'circumferential garment section preserves angular branch;20mm body allowance'})
 ...
 for j,t in enumerate(tvals):
  for a in u:
   ...
   if front:
    z+=.017*aa*(1-t)**4
    z-=.045*(1-aa)**1.4*t**5
    y=fp[j]-[.057,.057,.058,.058,.045,.040,.030,.022,.020][j]*aa**1.8+.008*(1-aa)
   else:
    z+=.018*(1-aa)*(1-t)**4
    z-=.016*(1-aa)*t**5
    y=bp[j]+[.042,.042,.037,.031,.026,.021,.014,.008,.007][j]*aa**1.65
   vs.append((w[j]*a,y,z))
  if j:
...
vs=[];fs=[];N=48
for j,z in enumerate([1.072,1.080,1.12,1.154,1.204,1.215]):
 for i in range(N):
  a=2*math.pi*i/N;direction=Vector((math.cos(a),math.sin(a),0));center=Vector((0,-.025,z));h=bodybv.ray_cast(center,direction,.30)
  if h[0] is None:raise RuntimeError('Missing central torso section')
  vs.append(h[0]+direction*.030)
 if j:
...
(R/'records/correction1_changes.json').write_text(json.dumps({'source':str(src),'sha256':hashlib.sha256(src.read_bytes()).hexdigest(),'garment_changes':changes,'neck_outer_loop_count':n,'body_edits':0,'new_parts':[o.name for o in owned]},indent=2))
bpy.ops.wm.save_as_mainfile(filepath=str(R/'ada_bw6_torso_correction1.blend'))
for v in ['front','profile','back','three_quarter']:
 s.camera=bpy.data.objects['BW4_Camera_'+v];s.render.filepath=str(R/'captures'/('correction1_'+v+'.png'));bpy.ops.render.render(write_still=True)
print('BW6_CORRECTION1_RENDERED')
