...
out=R/'ada_bw6_torso_sectioned_coat.blend';bpy.ops.wm.save_as_mainfile(filepath=str(out))
(R/'records/sectioned_coat.json').write_text(json.dumps({'source_object':source.name,'method':coat['BW6_method'],'changed_chest_vertices':changes,'source_neck_points':[list(p) for p in points],'unwrapped_monotone_angles':angles,'new_rings':ring_records},indent=2))
s.cycles.samples=12;s.render.threads=4
for view in ['front','profile','three_quarter','back']:
 s.camera=bpy.data.objects['BW4_Camera_'+view];s.render.filepath=str(R/'captures'/('sectioned_'+view+'.png'));bpy.ops.render.render(write_still=True)
print('BW6_SECTIONED_COAT_COMPLETE')
