...
R=Path(__file__).resolve().parents[1];src=R/'ada_bw6_torso_initial.blend'
...
(R/'records/initial_queries.json').write_text(json.dumps(out,indent=2))
...
for tag,keep in [('armor_only',[n for n in owned if n!='BW6_PaddedCoat_Tailored']),('coat_only',['BW6_PaddedCoat_Tailored']),('body_only',['BW4_CONTEXT_BW1_IndexedBody'])]:
 ...
 s.render.filepath=str(R/'captures'/('initial_'+tag+'.png'));bpy.ops.render.render(write_still=True)
