...
src=R/'ada_bw6_torso_clearance_work.blend';bpy.ops.wm.save_as_mainfile(filepath=str(src))
base=R.parents[1]
...
(R/'records/clearance_pose_probe.json').write_text(json.dumps({'source':str(src),'sha256':hashlib.sha256(src.read_bytes()).hexdigest(),'modifier_order_before':original,'modifier_order_after':[m.type for m in coat.modifiers],'frames':records},indent=2))
s.cycles.samples=12;s.render.threads=4;s.frame_set(1)
for view in ['front','profile','three_quarter']:
 s.camera=bpy.data.objects['BW4_Camera_'+view];s.render.filepath=str(R/'captures'/('clearance_'+view+'.png'));bpy.ops.render.render(write_still=True)
