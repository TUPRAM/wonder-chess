...
for side,angle in [('Rear',-35),('Front',35)]:
 ...
 for u in [.028,.044,.060,.075,.090,.105,.120,.132,.142,.152,.162,.174]:
  ...
  radius,target=min(hits)
  # Prebend the leather before the measured proximal edge of an overlapping plate.
  if u in [.028,.044,.120,.132]:
   for ahead in [.008,.014,.020]:
    for n,geom in g.items():
     hh,nn,tt,dd=geom[2].ray_cast(origin+axis*ahead,rad,.25)
     if hh is not None and dd<radius:radius,target=dd,n
  center=origin+rad*(radius-.004);vs.extend([center-tan*.006,center+tan*.006]);stations.append({'axis_m':u,'radius_m':radius,'limiting_plate':target,'center_m':list(center),'owner':parts[0].name if u<=.044 else parts[1].name})
 ...
 sub=o.modifiers.new('Flexible section interpolation','SUBSURF');sub.levels=2;sub.render_levels=2;sub.boundary_smooth='PRESERVE_CORNERS';wall=o.modifiers.new('2mm leather','SOLIDIFY');wall.thickness=.002;wall.offset=0;wall.use_even_offset=False;names.append(o.name);record.append({'object':o.name,'width_m':.012,'stations':stations,'method':'Rest sections ray toward armor interior; route4mm inward from innermost actual steel face. Independent Hook stations, no per-frame projection or metal deformation.'})
s['BW6_SHOULDER_SUPPORT_STUDY']=json.dumps(names);source=R/'ada_bw6_shoulder_understrap_prebent.blend';bpy.ops.wm.save_as_mainfile(filepath=str(source));(R/'records/understrap_prebent_construction.json').write_text(json.dumps(record,indent=2))
for f in [1,49,73,'lowered']:
 sc=ck.load(source);ck.render(sc,'understrap_prebent_shoulder_'+str(f),pose=f)
