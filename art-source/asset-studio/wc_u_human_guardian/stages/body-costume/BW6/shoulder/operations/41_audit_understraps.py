...
source=R/'ada_bw6_shoulder_understrap_initial.blend';s=ck.load(source);names=json.loads(s['BW6_SHOULDER_SUPPORT_STUDY']);metal=json.loads(s['BW6_SHOULDER_ACTIVE']);coat='BW6_PaddedCoat_Tailored';body='BW6_BodyFit_Candidate';records=[]
for fr in list(range(1,98))+[i+.5 for i in range(68,89)]+['lowered']:
 ...
 for n in names:
  ...
  bpy.context.view_layer.update();raw=ck.geom(o);centers=[(raw[0][2*i]+raw[0][2*i+1])*.5 for i in range(4)];length=sum(float(np.linalg.norm(centers[i+1]-centers[i])) for i in range(3))
...
(R/'records/understrap_initial_audit.json').write_text(json.dumps({'source':str(source),'sha256':hashlib.sha256(source.read_bytes()).hexdigest(),'scope':'119 sampled poses. All crossings reported; no terminal overlap automatically excluded. Native hooks then2subdivision and2mmwall. Nonadjacent transverse checks do not classify coplanar/tangent/containment.','frames':records},indent=2))
