...
source=R/'ada_bw6_shoulder_understrap_sectioned.blend';s=ck.load(source);names=json.loads(s['BW6_SHOULDER_SUPPORT_STUDY']);metal=json.loads(s['BW6_SHOULDER_ACTIVE']);coat='BW6_PaddedCoat_Tailored';body='BW6_BodyFit_Candidate';records=[]
...
(R/'records/understrap_sectioned_audit.json').write_text(json.dumps({'source':str(source),'sha256':hashlib.sha256(source.read_bytes()).hexdigest(),'scope':'119 sampled poses. All crossings reported; no terminal overlap automatically excluded. Native hooks then2subdivision and2mmwall. Nonadjacent transverse checks do not classify coplanar/tangent/containment.','frames':records},indent=2))
