...
import hashlib,json
R=Path(__file__).resolve().parents[1];review=R/'review/r003';review.mkdir(parents=True,exist_ok=True);motion=R/'motion/r003';record=json.loads((motion/'video_verification.json').read_text());assert hashlib.sha256(Path(record['source_movie']).read_bytes()).hexdigest()==record['sha256'];font=ImageFont.truetype('C:/Windows/Fonts/arial.ttf',18);small=ImageFont.truetype('C:/Windows/Fonts/arial.ttf',14);sheets=[]
for batch,start in enumerate(range(1,98,20),1):
 image=Image.new('RGB',(1600,1170),'#202226');draw=ImageDraw.Draw(image);draw.text((15,12),f'BW6 r003 actual encoded dual-view frames {start}-{min(start+19,97)} / AUTHORING ONLY',font=font,fill='white')
...
pairs=[('baseline_bw5r003_shoulder_lowered.png','r003_combined_shoulder_lowered.png'),('baseline_bw5r003_rear_three_quarter_73.png','r003_combined_rear_three_quarter_73.png')]
for row,pair in enumerate(pairs):
 for col,name in enumerate(pair):
  im=Image.open(R/'captures'/name).convert('RGB');comparison.paste(im,(col*800,60+row*830));draw.text((col*800+12,20+row*830),('BW5 r003 baseline context' if col==0 else 'BW6 r003 combined context')+(' / lowered' if row==0 else ' / frame73'),font=font,fill='white')
