...
for face in old.polygons:
 ...
 if row not in (0,rows-2):f.append(ids)
newrows=[]
for first,second,depth in [(0,1,.0028),(rows-2,rows-1,.003)]:
 loops=[[first*N+i for i in range(N)]]
 for t in (.25,.75):
  ids=[]
  for i in range(N):
   p=v[first*N+i].lerp(v[second*N+i],t);p.y-=depth;ids.append(len(v));v.append(p)
...
o['BW6_edge_construction']='Neck and hem overlay meshes retired. Connected raised surface bands replace two existing narrow strips; original lower control points and upward center hem preserved. Same single Solidify wall continues through edge bands.'
out=R/'ada_bw6_backplate_correction1.blend';s.camera=bpy.data.objects['BW4_Camera_back'];bpy.ops.wm.save_as_mainfile(filepath=str(out))
...
for fr in [1,49]:
 ...
 for view in ['back','profile','three_quarter']:
  s.camera=bpy.data.objects['BW4_Camera_'+view];s.render.filepath=str(R/'captures'/f'correction1_context_{view}_{fr}.png');bpy.ops.render.render(write_still=True)
s.frame_set(1)
for ob in s.objects:
...
for view in ['back','profile']:
 s.camera=bpy.data.objects['BW4_Camera_'+view];s.render.filepath=str(R/'captures'/f'correction1_pair_{view}.png');bpy.ops.render.render(write_still=True)
(R/'records/correction1_construction.json').write_text(json.dumps({'source':str(source),'source_sha256':hashlib.sha256(source.read_bytes()).hexdigest(),'old_control_positions_preserved':len(old.vertices),'new_band_rows':newrows,'new_vertices':len(v),'faces':len(f),'removed_overlay_trims':['BW6_Back_Neck_Rebuilt','BW6_Back_Hem_TurnedBorder'],'body_coat_front_other_geometry_changed':False,'method':'Topology-connected raised edge bands; no intersection exclusion and no pasted trim'},indent=2));print('BACKPLATE_CONNECTED_BANDS_BUILT')
