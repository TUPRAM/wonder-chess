...
objects=[bpy.data.objects['BW1_CoatUpper_Continuous']]
next(m for m in objects[0].modifiers if m.type=='SOLIDIFY').use_even_offset=False
...
(out/'coat_even_offset_false_temporal_audit.json').write_text(json.dumps(report,indent=2));print(json.dumps({'extreme_events':len(report['extreme_vertex_events']),'temporal_flags':report['temporal_flags']}),flush=True)
