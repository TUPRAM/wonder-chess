...
source=out.parent/'ada_body_costume_checkpoint_r002_ART_REVISE.blend'
expected='9e15e8446afbb6f60725e3d2d72026df07f707271b007e5f31c2919f565f30ef'
...
video.render.filepath=str(out/'ada_bw1_articulation_r002_review.mp4')
...
metadata['status']='VIDEO_ENCODED';metadata['video_file']=str(out/'ada_bw1_articulation_r002_review.mp4');metadata['encoding']='Blender native VSE, H264 in MPEG4, no audio';metadata['source_sha256_after']=hashlib.sha256(source.read_bytes()).hexdigest();metadata['finished_utc']=time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime())
(out/'metadata.json').write_text(json.dumps(metadata,indent=2))
