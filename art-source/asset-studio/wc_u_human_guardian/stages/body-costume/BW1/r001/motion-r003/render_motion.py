...
video.render.filepath=str(out/'ada_bw1_articulation_r003_review.mp4')
...
metadata['status']='VIDEO_ENCODED';metadata['video_file']=str(out/'ada_bw1_articulation_r003_review.mp4');metadata['encoding']='Blender native VSE, H264 in MPEG4, no audio';metadata['source_sha256_after']=hashlib.sha256(source.read_bytes()).hexdigest();metadata['finished_utc']=time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime())
(out/'metadata.json').write_text(json.dumps(metadata,indent=2))
