...
for side,h in handles.items():
    h['initial_center_world']=h['center_world']
    h['center_world']=list(Vector(h['center_world'])+Vector(h['forward_world'])*.013+Vector(h['palmar_world'])*.011)
    h['placement_correction']='One documented setup adjustment before final grip fit: +13 mm finger-forward, +11 mm palmar; unchanged axis, diameter and length. Fixed across final poses.'
    obj=bpy.data.objects['STUDY_FIXED_HANDLE_'+side];obj.location=h['center_world']
    bpy.context.view_layer.update();h['matrix_world']=[list(row) for row in obj.matrix_world]
...
record['status']='CORRECTION_2_PENDING_IMAGE_REVIEW';record['correction']='Multiple bounded initial chains and stronger cylinder clearance; fixed handle unchanged.'
(out/'poses_correction2.json').write_text(json.dumps(record,indent=2))
for name,vals in {**record['poses']['sword_grip'],**record['poses']['shield_grip']}.items():rig.pose.bones[name].rotation_euler=vals
bpy.context.view_layer.update()
bpy.ops.wm.save_as_mainfile(filepath=str(out/'hand_pose_correction2.blend'))
