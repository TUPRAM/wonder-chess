...
scene.render.filepath=str(out/'frame129_fresh_process_recheck.png');bpy.ops.render.render(write_still=True,scene=scene.name)
...
for obj in scene.objects:
    ...
    ev.to_mesh_clear()
(out/'frame129_geometry_screen.json').write_text(json.dumps({'frame':129,'world_extent_screen':'abs(x)>1.4 or abs(y)>1.4 or z>2.2 or z<-0.3','observations':observations},indent=2))
