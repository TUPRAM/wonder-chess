...
next(m for m in bpy.data.objects['BW1_Leggings'].modifiers if m.type=='SOLIDIFY').use_even_offset=False
scene.frame_set(129);bpy.context.view_layer.update()
scene.render.filepath=str(out/'frame129_even_offset_false_UNSAVED.png');bpy.ops.render.render(write_still=True,scene=scene.name)
...
for obj in scene.objects:
    ...
    ev.to_mesh_clear()
(out/'frame129_even_offset_false_screen.json').write_text(json.dumps({'frame':129,'world_extent_screen':'abs(x)>1.4 or abs(y)>1.4 or z>2.2 or z<-0.3','observations':observations},indent=2))

scene.frame_set(159);bpy.context.view_layer.update();scene.render.filepath=str(out/'frame159_even_offset_false_UNSAVED.png');bpy.ops.render.render(write_still=True,scene=scene.name)
