"""Native Blender VSE reopen and full96-frame decode; never saves a blend."""
import bpy
import hashlib
import json
...
out = Path(__file__).resolve().parents[1] / 'motion'
video = out / 'BW4_fixed_hand_LOCAL_TURNTABLE_ART_REVISE.mp4'
expected = json.loads((out.parent/'records/final_capture_manifest.json').read_text())['local_turntable']['movie_sha256']
...
data = {'file': str(video), 'bytes': video.stat().st_size, 'sha256': digest,
        ...
        'native_png_count': len(list((out / 'local_turntable_frames').glob('frame_*.png'))),
        'scope': 'LOCAL CAMERA TURNTABLE ONLY; NOT GAME CLIPS',
...
files = [decoded / f'frame_{frame:04}.png' for frame in range(1, 97)]
...
data['status'] = 'ALL96_FRAMES_DECODED_24FPS_800x800'
