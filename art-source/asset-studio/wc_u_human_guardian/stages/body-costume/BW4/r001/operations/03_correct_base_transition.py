"""First substantive correction: connected palm displacement field and local web cage reconstruction."""
...
for i,(v,w) in enumerate(zip(v0,d['weights'])):
 ...
 if fam[n]>.88 and along>(.028 if n==1 else .007):
  fixed[i]=True
  if n==1:
   oldlength=np.linalg.norm(np.diff(points[n],axis=0),axis=1).sum();a=along/oldlength*thumbdist[-1];ii=int(np.clip(np.searchsorted(thumbdist,a),1,len(thumbseq)-1));q=thumbseq[ii];nt=thumbseq[ii]-thumbseq[ii-1];nt/=np.linalg.norm(nt);nz=np.array([0.,1,0]);nz-=nt*np.dot(nz,nt);nz/=np.linalg.norm(nz);nx=np.cross(nt,nz);target[i]=q+nx*dx+nz*dz
  else:
 ...
 elif v[1]<.026 or (v[1]<.048 and v[0]<.012):
  fixed[i]=True;target[i,2]+=.003*math.exp(-((v[0]+.004)/.04)**2-((v[1]-.055)/.04)**2)
adj=[set() for _ in v0]
for f in d['faces']:
...
me=bpy.data.meshes.new('BW4_Rebuilt_Connected_Base_Cage');me.from_pydata(verts.tolist(),[],d['faces']);me.update();o.data=me;o.name='BW4_ClosedGlove_Correction1'
...
o['method']='Fixed distal cages with one connected local palm displacement solve, web faces poked/reconnected across original edges'
o['revision']='First substantive correction; not a new attempt counter'
s['BW4_scope']='CORRECTION1_LOCAL_REVIEW_REQUIRED'
for name in ['palm','dorsal','side','underside','oblique','axial']:
 ...
 if name in ['side','underside','oblique']:
  ...
  s.render.filepath=str(OUT/'captures'/('correction1_'+name+'_hidden.png'));bpy.ops.render.render(write_still=True)
s.camera=bpy.data.objects['BW4_oblique'];sub=o.modifiers[0];sub.show_render=False
w=o.modifiers.new('Actual cage','WIREFRAME');w.thickness=.00032;w.use_replace=False;s.render.filepath=str(OUT/'captures/correction1_cage.png');bpy.ops.render.render(write_still=True);o.modifiers.remove(w);sub.show_render=True
for ob in s.objects:
 if ob.name.startswith('BW4_SelectedSword_'):ob.hide_render=False
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'ada_fixed_hand_correction1.blend'))
(OUT/'records/correction1.json').write_text(json.dumps({'fixed_vertices':int(fixed.sum()),'local_free_transition_vertices':len(free),'source_web_faces_replaced':oldcount,'candidate_vertices':len(me.vertices),'candidate_faces':len(me.polygons),'predeclared_contact_source_vertices':predeclared,'no_source_mesh_modified':True},indent=2))
print('CORRECTION1',len(me.vertices),len(me.polygons),'REBUILT_WEB_FACES',oldcount)
