...
R=Path('C:/Users/iputu/Documents/Wonder Chess/art-source/asset-studio/wc_u_human_guardian/stages/body-costume/BW4/r001/armor');v=R/'motion/BW4_ARMOR_AUTHORING_DIAGNOSTIC_NOT_GAME_CLIPS.mp4';out=R/'motion/decoded';out.mkdir(exist_ok=True)
s=bpy.context.scene;e=s.sequence_editor_create();strip=e.strips.new_movie('Actual encoded BW4 movie',str(v),channel=1,frame_start=1)
...
(R/'motion/video_verification.json').write_text(json.dumps({'source_movie':str(v),'sha256':hashlib.sha256(v.read_bytes()).hexdigest(),'frames_decoded':97,'dimensions':[800,800],'fps':24,'method':'Native Blender VSE reopen and decode every integer frame to PNG, fresh factory scene.','status':'DECODED_NOT_ART_APPROVED','decoded_hashes':{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in files}},indent=2))
print('ALL97_DECODED')
