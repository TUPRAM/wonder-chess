...
assert not bpy.data.objects['HP1_HEAD_POLISH_Head_r015'].hide_render
for label in ['front','profile','three_quarter','primary_fit']:
    ...
    s.render.filepath=ROOT+'/captures/r015_'+label+'.png';bpy.ops.render.render(write_still=True)
...
try:
    ...
    s.camera=bpy.data.objects[s.name+'_three_quarter'];s.render.filepath=ROOT+'/captures/r015_reverse_key.png';bpy.ops.render.render(write_still=True)
finally:
    key.matrix_world=km;fill.matrix_world=fm
try:
    for side in ['R','L']:bpy.data.objects[s.name+'_Eye_'+side].hide_render=True
    s.camera=bpy.data.objects[s.name+'_front'];s.render.filepath=ROOT+'/captures/r015_openings.png';bpy.ops.render.render(write_still=True)
finally:
    for side in ['R','L']:bpy.data.objects[s.name+'_Eye_'+side].hide_render=False
s.camera=bpy.data.objects[s.name+'_primary_fit']
print('LOCAL r015 six actual captures written; baseline camera and light transforms retained.')
