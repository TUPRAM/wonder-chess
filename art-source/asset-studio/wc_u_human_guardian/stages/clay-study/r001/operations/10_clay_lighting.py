...
mat=bpy.data.materials.new('REVIEW_ONLY_Neutral_Clay');mat.use_nodes=True
bsdf=mat.node_tree.nodes.get('Principled BSDF');bsdf.inputs['Base Color'].default_value=(.46,.46,.46,1);bsdf.inputs['Roughness'].default_value=.78
for o in scene.objects:
    if o.type=='MESH' and o.name.startswith('ADA_') and not o.hide_render:
        o.data.materials.clear();o.data.materials.append(mat)
scene.world.use_nodes=True;scene.world.node_tree.nodes['Background'].inputs['Color'].default_value=(.12,.12,.12,1)
scene.world.node_tree.nodes['Background'].inputs['Strength'].default_value=.6
for name,pos,power,size in [('Key',(-1.2,2.0,2.8),180,1.8),('Fill',(1.3,1.4,1.9),90,1.5),('Rim',(.1,-1.4,2.3),150,1.4)]:
    d=bpy.data.lights.new('CLAY_'+name,'AREA');d.energy=power;d.shape='DISK';d.size=size
    o=bpy.data.objects.new('CLAY_'+name,d);collection('STUDY_REVIEW').objects.link(o);o.location=pos;o.rotation_euler=(Vector((0,0,1.5))-o.location).to_track_quat('-Z','Y').to_euler()
scene.view_settings.view_transform='AgX'
for area in bpy.context.screen.areas:
    if area.type=='VIEW_3D':area.spaces.active.shading.show_shadows=False
scene.camera=bpy.data.objects['CAM_three_quarter']
scene.render.filepath='C:/Users/iputu/Documents/Wonder Chess/art-source/asset-studio/wc_u_human_guardian/stages/clay-study/r001/captures/three_quarter_cycles_r002.png'
bpy.ops.render.render(write_still=True)
print('Path-traced neutral clay diagnostic captured to distinguish open-surface Workbench shadow artifacts from geometry.')
