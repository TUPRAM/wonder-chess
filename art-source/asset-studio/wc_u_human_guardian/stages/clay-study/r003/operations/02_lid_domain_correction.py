...
assert bpy.context.scene.get('as1_operation_id')=='clay-r003-spherical-lid-proof'
head=bpy.data.objects['ADA_Head_Continuous'];cx=.035;cz=1.688;cy=.026;radius=.023
# The prior proof applied the spherical square root outside its useful domain.
# Refit only inserted annulus rows; exterior surface and x/z coordinates stay fixed.
...
while boundary:
    ...
    loops.append(loop)
for inner in loops:
    s=1 if sum(v.co.x for v in inner)>0 else -1
    prev=set();curr=set(inner)
    # Breadth-first rings correspond to the 14 inserted patch rows.
    rim_y={v: v.co.y for v in inner}
    for level in range(14):
        u=1-level/14;blend=u*u*(3-2*u)
        for v in curr:
            dx=v.co.x-s*cx;dz=v.co.z-cz-s*.085*dx
            a=math.atan2(dz/.0083 if dz>=0 else dz/.0058,dx/.0185)
            ix=.0185*cos(a);iz=(.0083 if sin(a)>0 else .0058)*sin(a)+s*.085*ix
            iy=cy+sqrt(max(0,radius**2-ix**2-iz**2))+.0007
            base=face_y(v.co.x,v.co.z)
            v.co.y=base*(1-blend)+iy*blend+(.0018 if sin(a)>0 else .0008)*sin(pi*u)**2
        nxt={e.other_vert(v) for v in curr for e in v.link_edges if e.other_vert(v) not in curr and e.other_vert(v) not in prev}
        prev.update(curr);curr=nxt
bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.to_mesh(head.data);bm.free();head.data.update()
bpy.context.scene['as1_operation_id']='clay-r003-lid-domain-correction'
bpy.ops.wm.save_as_mainfile(filepath=bpy.data.filepath)
for name in ['face','profile']:
    bpy.context.scene.camera=bpy.data.objects['CAM_'+name]
    bpy.context.scene.render.filepath='C:/Users/iputu/Documents/Wonder Chess/art-source/asset-studio/wc_u_human_guardian/stages/clay-study/r003/captures/'+name+'_lid_corrected.png'
    bpy.ops.render.render(write_still=True)
print('Only annular lid depth refitted; no full-face smoothing.')
