...
assert bpy.context.scene.get('as1_operation_id')=='clay-r002-review'
bpy.data.objects['ADA_Hair_Overlapping_Sculpt'].hide_render=True
bpy.data.objects['ADA_Hair_Overlapping_Sculpt'].hide_set(True)
for o in collection('STUDY_HAIR').objects:
    if o.name.startswith('ADA_Overlapping_Lock_') or o.name=='ADA_Hair_Foundation':
        o.hide_render=False;o.hide_set(False)
def samples(points,n=70):
    ps=[]
    for i in range(n):
...
for sign in (-1,1):
    for k in range(4):
        off=k*.013
        points=[(sign*.087,.023,1.700+off),(sign*.102,-.018,1.708+off),(sign*.092,-.070,1.710+off),(sign*.060,-.113,1.690+off*.5),(sign*.012,-.14,1.651)]
        ps=samples(points);vs=[];S=16
        for i,p in enumerate(ps):
            ...
            r=.0005+.012*(sin(pi*u)**.5)
            for j in range(S):
                a=2*pi*j/S;vs.append(tuple(p+side*r*cos(a)+normal*r*.85*sin(a)))
        ...
        mesh('ADA_Temporal_Sweep_%s_%s'%(sign,k),vs,fs,'STUDY_HAIR')
scene=bpy.context.scene;scene['as1_operation_id']='clay-r002-temporal-coverage'
...
for name in ['front','profile','back','face','three_quarter']:
    ...
    scene.render.filepath='C:/Users/iputu/Documents/Wonder Chess/art-source/asset-studio/wc_u_human_guardian/stages/clay-study/r002/captures/'+name+'_final.png'
    bpy.ops.render.render(write_still=True)
scene.camera=bpy.data.objects['CAM_three_quarter']
bpy.ops.wm.save_as_mainfile(filepath=bpy.data.filepath)
print('Temporal hair flow added across exposed side foundation; final study captures created.')
