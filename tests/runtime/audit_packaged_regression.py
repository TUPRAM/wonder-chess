"""Audit actual packaged Shipping regression against preserved engine and native rows."""
...
def main():
    ...
    report_path = args.evidence_directory.resolve() / "regression.json"
    provenance_path = Path(load(report_path.parent / "launch.json")["provenance_path"])
    ...
    check("captured_process_exit_zero", observed_exit.get("exit_code") == 0, observed_exit)
    exit_lines = []
    log_errors = []
    ...
    result = {"utc": datetime.now(timezone.utc).isoformat(), "status": "PASS" if not failures else "FAIL",
              ...
              "exit_boundary": "The separately captured process record reports exit_code0. No Shipping game.log is present, so engine log error/exit checks remain NOT_RUN. This headless regression overlapped the beginning of the two-process functional network run and is not a rendering performance benchmark.",
    ...
    if failures:
        raise SystemExit("Actual audit failed; inspect regression-analysis.json: " + ", ".join(failures))
    ...
    lines += ["", "Runtime-loaded profile and all five canonical data hashes agree with the bound immutable provenance manifest. The actual Shipping inner executable SHA256 matches that immutable manifest.", "",
              result["exit_boundary"], "",
              "This audit binds the actual Shipping execution to its immutable source/data/content/package manifest. The earlier Shipping executions remain preserved separately; this headless run does not measure TAA rendering.", ""]
